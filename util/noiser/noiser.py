import torch
# from transformer import EmbeddingDict, instantiate_module
...
class BaseNoiser(torch.nn.Module):
    def __init__(self, args, device):
        ...
        self.training_timestep_map = torch.arange(start=0, end=self.training_timesteps, step=1, dtype=torch.int64,
                                                  device=self.device)
        self.inference_timesteps = self.args.inference_timesteps
        self.inference_timestep_map = torch.arange(start=0, end=self.training_timesteps,
                                                   step=int(self.training_timesteps / self.inference_timesteps),
    ...
    def trajectory_dsb(self, x_0, x_1, sample=False):
        self.prepare_gamma_dsb()  # 准备gamma参数
        ...
        x_cache = torch.stack([x_1] + x_cache, dim=0) if sample else torch.cat(x_cache, dim=0).cpu()
        gt_cache = torch.cat(gt_cache, dim=0)
        t_cache = torch.cat(t_cache, dim=0)
...
class DSBNoiser(BaseNoiser):
    def __init__(self, *args, mean=0, std=1, **kwargs):
        ...
        # 计算gamma的累加和，并归一化到[0, 1]
        self.prepare_gamma_dsb()
        self.gamma_cumsum = torch.cumsum(
            torch.cat([torch.zeros(size=(1,), dtype=torch.float32, device=self.device), self.gammas], dim=0), dim=0)
    ...
    def coefficient(self, t):
        ...
        if tmax >= len(self.timestep_map):
            ...
            ret = {
                'coef0': 1 - self.gamma_cumsum[t],
                'coef1': self.gamma_cumsum[t],
            }
        ret = {k: v.float() if isinstance(v, torch.Tensor) else v for k, v in ret.items()}
        return ret
    ...
    def trajectory(self, x_0, x_1, sample=False):
        ...
        if sample:
            ...
            x_cache = torch.cat(x_cache, dim=0)
        gt_cache = torch.cat(gt_cache, dim=0)
        t_cache = torch.cat(t_cache, dim=0)
        return x_cache, gt_cache, t_cache
