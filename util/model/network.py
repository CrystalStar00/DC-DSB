...
class Conv1d(torch.nn.Module):  # 定义一个继承自 torch.nn.Module 的 1D 卷积类
    def __init__(self,
                 in_channels, out_channels, kernel, bias=True, up=False, down=False,
                 resample_filter=[1, 1], fused_resample=False, init_mode='kaiming_normal', init_weight=1, init_bias=0,
                 ):
        # 断言检查，确保不会同时执行上采样和下采样
        assert not (up and down)
        super().__init__()  # 调用父类的构造函数
        self.in_channels = in_channels  # 输入通道数
        self.out_channels = out_channels  # 输出通道数
        self.up = up  # 是否执行上采样
        self.down = down  # 是否执行下采样
        self.fused_resample = fused_resample  # 是否使用融合采样

        # 初始化权重参数，使用用户指定的初始化方式
        ...
        ) if kernel else None  # 如果有 kernel，初始化权重；否则置为 None

        self.bias = torch.nn.Parameter(
            weight_init([out_channels], **init_kwargs) * init_bias
        ) if kernel and bias else None  # 如果有 kernel 且 bias 为 True，则初始化偏置；否则置为 None

        # 初始化重采样滤波器，用于上采样或下采样
        f = torch.as_tensor(resample_filter, dtype=torch.float32).unsqueeze(0).unsqueeze(1)
        f = f / f.sum()  # 对滤波器进行归一化处理
        self.register_buffer('resample_filter', f if up or down else None)  # 将滤波器注册为模型的缓冲区变量

    def forward(self, x):  # 前向传播函数，定义输入 x 如何经过网络计算得到输出
        # 将权重、偏置和重采样滤波器的数据类型转换为与输入一致的类型
        w = self.weight.to(x.dtype) if self.weight is not None else None
        b = self.bias.to(x.dtype) if self.bias is not None else None
        f = self.resample_filter.to(x.dtype) if self.resample_filter is not None else None

        # 计算权重和滤波器的填充大小（用于保持输出与输入尺寸一致）
        w_pad = w.shape[-1] // 2 if w is not None else 0
        f_pad = (f.shape[-1] - 1) // 2 if f is not None else 0
        ...
        if self.fused_resample and self.up and w is not None:
            ...
            x = torch.nn.functional.conv_transpose1d(
                x, f.mul(2).tile([self.in_channels, 1, 1]), groups=self.in_channels, stride=2,
        ...
        # 如果启用了融合采样，并且需要下采样
            ...
            x = torch.nn.functional.conv1d(
                x, f.tile([self.out_channels, 1, 1]), groups=self.out_channels, stride=2
            )

        # 如果没有使用融合采样，则根据需要分别进行上采样或下采样
        else:
            if self.up:
                # 上采样：使用卷积转置层
                x = torch.nn.functional.conv_transpose1d(
                    x, f.mul(2).tile([self.in_channels, 1, 1]), groups=self.in_channels, stride=2, padding=f_pad
                )
            if self.down:
                # 下采样：普通卷积操作
                x = torch.nn.functional.conv1d(
                    x, f.tile([self.in_channels, 1, 1]), groups=self.in_channels, stride=2, padding=f_pad
                )
            if w is not None:
                # 如果有权重参数，则进行普通卷积
        ...
        # 如果有偏置参数，则将偏置添加到输出中
        if b is not None:
            x = x.add_(b.reshape(1, -1, 1))  # 将偏置广播并添加到输出
...
class UNetBlock(torch.nn.Module):
    def __init__(self,
        ...
        # 第一个卷积层，支持上采样、下采样和其他初始化选项
        self.conv0 = Conv2d(in_channels=in_channels, out_channels=out_channels, kernel=3, up=up, down=down, resample_filter=resample_filter, **init)
        # 线性层用于处理嵌入输入，将其转换为适合卷积层的参数
...
class TimeEmbedding(nn.Module):
    ...
    def forward(self, t):
        ...
        return self.time_emb(t)  # [batch_size, time_dim]
...
class UNet1D(nn.Module):
    ...
    def __init__(self, input_dim=5045, hidden_dim=256, time_dim=128):
        super().__init__()


        self.time_embedding = TimeEmbedding(time_dim)

        self.enc1 = DoubleConv1D(1, hidden_dim)
        self.enc2 = DoubleConv1D(hidden_dim, hidden_dim)

        self.dec1 = DoubleConv1D(hidden_dim + time_dim, hidden_dim)
        self.dec2 = DoubleConv1D(hidden_dim, 1)

    def forward(self, x, t):
        x=x.unsqueeze(1)

        x1 = self.enc1(x)  # [batch_size, hidden_dim, 5045]
        x2 = self.enc2(x1)  # [batch_size, hidden_dim, 5045]

        t_emb = self.time_embedding(t)  # [batch_size, time_dim]
        t_emb = t_emb.unsqueeze(-1).expand(-1, -1, x2.size(-1))  # [batch_size, time_dim, 5045]

        x = torch.cat([x2, t_emb], dim=1)  # [batch_size, hidden_dim + time_dim, 5045]
        x = self.dec1(x)  # [batch_size, hidden_dim, 5045]
        x = self.dec2(x)  # [batch_size, 1, 5045]
        x=x.squeeze(1)
        return x
