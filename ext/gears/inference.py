...
def evaluate(loader, model, uncertainty, device):
    ...
    for batch in tqdm.tqdm(loader,desc="evaluate:"):

        batch.to(device)
        # x=batch["x"] = batch["x"].to(device)
        # y = batch["y"] = batch["y"].to(device)
        # cond = batch["cond"]
        # batch["de_idx"] = batch["de_idx"].to(device)
        ...
        with torch.no_grad():
            if uncertainty:
                ...
                p = model(batch)
            t = batch.y
            batch_size = len(batch)  # 128
            nodes_per_graph = batch.x.size(0) // batch_size  # 5012
            x = batch.x.view(batch_size, nodes_per_graph)
            pred.extend(p.cpu())
            truth.extend(t.cpu())
            ctrl.extend(x.cpu())
            pert_idx = batch.pert_idx
            true_conds.extend(pert_idx)
    ...
    true_conds=torch.tensor(true_conds)

    results['pred']= pred.detach().cpu()
    results['truth']= truth.detach().cpu()
    results['ctrl']=ctrl.detach().cpu()
    results['true_conds']=true_conds.cpu()
