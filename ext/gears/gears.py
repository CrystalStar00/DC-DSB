...
from .dataset.base import cteate_dateloader
...
class GEARS:
    ...
    def __init__(self, data_name,
        ...
        if self.weight_bias_track:
            import wandb
            wandb.init(project=proj_name, name=exp_name,mode="offline")
            self.wandb = wandb
        else:
            self.wandb = None
        ...
        pert_data = cteate_dateloader(self.data_name, batch_size)
        # pert_data=data_name
        ...
        # self.gene_list = self.dataloader["train_loader"].dataset.gene_names.values.tolist()
    ...
    def model_initialize(self, hidden_size = 64,
        ...
        if self.wandb:
            self.wandb.config.update(self.config,allow_val_change=True)
    ...
    def train(self, epochs = 20,
        ...
        for epoch in range(epochs):
            ...
            for step, batch in enumerate(train_loader):
                ...
                # y=batch["y"]=batch["y"].to(self.device)
                # cond=batch["cond"]
                # batch["de_idx"]=batch["de_idx"].to(self.device)
                ...
                if self.config['uncertainty']:
                    ...
                    # loss = uncertainty_loss_fct(pred, logvar, y, cond["pert"],
                    #                             reg=self.config['uncertainty_reg'],
                    #                             ctrl=self.ctrl_expression,
                    #                             dict_filter=self.dict_filter,
                    #                             direction_lambda=self.config['direction_lambda'])
    ...
    def test(self):
        ...
        pred=test_res['pred']
        ctrl=test_res['ctrl']
        true_conds=test_res['true_conds']
        gene_names=test_loader.dataset.gene_names

        extras=test_loader.dataset.extras
        de_gene_idx_dict = None if extras is None else extras.get("rank_genes_groups_cov_all_idx_dict")  # 基因组的协方差索引字典
        ndde20_idx_dict = None if extras is None else extras.get("top_non_dropout_de_20")  # 非dropout的top 20基因索引字典

        # gene_name=
        _, scores = perturbation_eval(true, pred, ctrl, gene_names=gene_names, de_gene_idx_dict=de_gene_idx_dict,
                                      true_conds=true_conds, ndde20_idx_dict=ndde20_idx_dict,
                                      path_to_save=f'{self.data_name}_perturb.png')

        for m,v in scores.items():
            log=f"test_{m}:{v}\n"
            print(log)
            if self.wandb:
                self.wandb.log({self.data_name:log})
            _
    #             if self.wandb:
